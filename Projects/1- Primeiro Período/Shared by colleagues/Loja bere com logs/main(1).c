...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------//
    Outro* criarOutro(int dado) { // criar um novo nó duplo
    Outro* novoOutro = (Outro*) malloc(sizeof(Outro));
    if (novoOutro == NULL) {
        perror("Erro ao alocar memória para novo item da lista 'Outros'");
        exit(EXIT_FAILURE);
    }
    novoOutro->dado = dado;
...
void inserirNoInicioOutro(Outro** inicio, int dado) { // inserção no inicio
    ...
    *inicio = novoOutro;
    printf("Inserido %d no início (categoria 'Outros').\n", dado);
}

void inserirNoFinalOutro(Outro** inicio, int dado) { // inserção no final
    Outro* novoOutro = criarOutro(dado);
    if (*inicio == NULL) {
        *inicio = novoOutro;
    } else {
...
int main()
{
    Outro* inicio = NULL; // Lista começa vazia
    ...
    setlocale(LC_ALL, "Portuguese"); // Para acentuação funcionar no Windows
