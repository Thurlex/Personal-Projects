...
fopen(NOME_ARQUIVO, "r");
    ...
    while (fscanf(arquivo, "%d;%49[^;];%f;%d;%d\n", &produtos[contador].Id, produtos[contador].Nome, &produtos[contador].preco, &produtos[contador].quantidade, &produtos[contador].id_categoria) == 5) {
        contador++;
    }
    fclose(arquivo);

    // Ordena os produtos por ID
    ...
    // Reescreve os produtos ordenados no arquivo
    ...
    for (int i = 0; i < contador; i++) {
        fprintf(arquivo, "%d;%s;%.2f;%d;%d\n", produtos[i].Id, produtos[i].Nome, produtos[i].preco, produtos[i].quantidade, produtos[i].id_categoria);
    }
...
void cadastrarProduto(PRODUTO produtos[MAX_PRODUTOS], int *contador)
{
    ...
    while (1)
    {
        ...
        salvarProduto(produtos[*contador]);    // Salva no arquivo
        (*contador)++;
        ...
        if (opcao == 1)
        {
            do
            {
                ...
                case 1:
                    ...
                    produtos[contador].id_categoria = ID_LIMPEZA;
                    break;
                case 2:
                    escrever_log("Opção selecionada: %d", escolha_acrescenta_produtos);
                    cadastrarProduto(produtos, &contador);
                    produtos[contador].id_categoria = ID_ALIMENTOS;
                    break;
                case 3:
                    escrever_log("Opção selecionada: %d", escolha_acrescenta_produtos);
                    cadastrarProduto(produtos, &contador);
                    produtos[contador].id_categoria = ID_PADARIA;
                    break;
                case 4:
                    escrever_log("Opção selecionada: %d", escolha_acrescenta_produtos);
                    cadastrarProduto(produtos, &contador);
                    produtos[contador].id_categoria = ID_OUTROS;
                    break;
                default:
