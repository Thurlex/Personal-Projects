...
void listarProdutos(noDuplo *produtos, const char *nomearquivo, const int id_categoria) { // Versão anterior apresentava int para *nomearquivo
    FILE *arquivo = fopen(nomearquivo, "r");
    PRODUTO temp;
    PRODUTO *produtosArray = NULL; // Array para armazenar produtos
    ...
    // Conta o número de produtos no arquivo
    while (fscanf(arquivo, "%d;%49[^;];%f;%d;%d\n", &temp.Id, temp.Nome, &temp.preco, &temp.quantidade, &temp.id_categoria) == 5) {
        count++;
    }
    rewind(arquivo); // Volta o ponteiro do arquivo para o início

    // Aloca a memória para o array de produtos
    produtosArray = (PRODUTO*)malloc(count * sizeof(PRODUTO));
    if (!produtosArray) {
        escrever_log("Erro ao alocar memória");
        fclose(arquivo);
        return;
    }

    // Guarda os produtos no array
    for (int i = 0; i < count; i++) {
        fscanf(arquivo, "%d;%49[^;];%f;%d;%d\n", // Lê cada produto do arquivo
               &produtosArray[i].Id, // Lê o ID do produto
               produtosArray[i].Nome, // Lê o nome do produto
               &produtosArray[i].preco, // Lê o preço do produto
               &produtosArray[i].quantidade, // Lê a quantidade do produto
               &produtosArray[i].id_categoria); // Lê a categoria do produto
    }
    fclose(arquivo);

    // Abre a função quicksort
    quicksort(produtosArray, 0, count - 1);

    // Exibe os produtos ordenados com seus títulos atribuidos
    ...
    printf("\n--- Lista de Produtos de %s (Ordenada por Nome) ---\n\n", titulo);
    printf("ID | Nome                          | Preço   | Qtde | Categoria\n");
    printf("---------------------------------------------------------------\n");

    for (int i = 0; i < count; i++) {
        printf("%d | %s | R$%f | %d | %d\n", // %2d para ID pois é pequeno, %-30s para nome com espaçamento fixo, R$%6.2f para preço com 2 casas decimais, %5d para quantidade
               produtosArray[i].Id, 
               produtosArray[i].Nome, 
               produtosArray[i].preco, 
               produtosArray[i].quantidade, 
               produtosArray[i].id_categoria);
    }

    free(produtosArray);
}

void swap(PRODUTO* a, PRODUTO* b) { // Função para trocar dois produtos
    PRODUTO t = *a; // Cria uma variável temporária para armazenar o produto a
    *a = *b; // Troca o produto a com o produto b
    *b = t; // Atribui o produto temporário t ao produto b
}

int particao(PRODUTO arr[], int low, int high) {
    char pivo[MAX_NOME]; // Variável para armazenar o nome do pivô
    strcpy(pivo, arr[high].Nome); // Pega o último elemento como pivô
    int i = (low - 1); // Índice do menor elemento

    for (int j = low; j <= high - 1; j++) { // Percorre o array
        if (strcmp(arr[j].Nome, pivo) < 0) { // Compara os nomes dos produtos
...
void quicksort(PRODUTO arr[], int low, int high) { // Função de ordenação QuickSort
    if (low < high) {
...
int Adicionar_Carrinho(float *valor_total_compra, float *valor_total_limpeza, float *valor_total_alimentos, float *valor_total_Padaria, float *valor_total_Outros, float *valor_caixa, float *valor_inicial_caixa, noDuplo *cabeca)
{
    ...
    while(1)
    {
        ...
        {
        case 1:
            Adicionar_Carrinho(&valor_total_compra,&valor_total_limpeza,&valor_total_alimentos,&valor_total_Padaria,&valor_total_Outros,&valor_caixa,&valor_inicial_caixa,&listarProdutos);
            break;
        case 2:
            return;
        default:
