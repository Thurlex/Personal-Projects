...
void liberarlista(noDuplo **cabeca) // Libera a memória alocada para a lista duplamente encadeada
{
    noDuplo* atual = *cabeca; // Inicia o nó atual como a cabeça da lista
    noDuplo* proximoNo; // Variável para armazenar o próximo nó
    while(atual != NULL)
    {
...
void trocar(noDuplo *a, noDuplo *b)
{
    PRODUTO temp = a->produto;
    a->produto = b->produto;
    b->produto = temp;
}

void ordenarNome(noDuplo *cabeca)
{
    if (cabeca == NULL) return;
    noDuplo *i, *j;
    for (i = cabeca; i != NULL; i = i->proximo)
    {
        for (j = i->proximo; j != NULL; j = j->proximo)
        {
            if (strcmp(i->produto.Nome, j->produto.Nome) > 0)
            {
...
void ordenarPrecoCrescente(noDuplo *cabeca)
{
    if (cabeca == NULL) return;
    noDuplo *i, *j;
    for (i = cabeca; i != NULL; i = i->proximo)
    {
        for (j = i->proximo; j != NULL; j = j->proximo)
        {
            if (i->produto.preco > j->produto.preco)
            {
                PRODUTO temp = i->produto;
                i->produto = j->produto;
                j->produto = temp;
...
void ordenarPrecoDecrescente(noDuplo *cabeca)
{
    if (cabeca == NULL) return;
    noDuplo *i, *j;
    for (i = cabeca; i != NULL; i = i->proximo)
...
void listarProdutos(noDuplo *produtos, const int *nomearquivo, const int id_categoria)
{
    ...
    {
        printf("%2d | %-30s | R$%6.2f | %5d | %d\n", temp.Id, temp.Nome, temp.preco, temp.quantidade, temp.id_categoria);
    }
...
noDuplo* buscarProdutoComCache(noDuplo **cabeca, int id_buscado, const char *nome_arquivo)
{
    ...
    if (buscarAquivo(&temp, id_buscado, nome_arquivo))
    {
        ...
        return buscarMemoria(*cabeca, id_buscado);
    }
...
noDuplo* carregarProdutosArquivo(const char *nome_arquivo)
{
    FILE *arquivo = fopen(nome_arquivo, "r");
    if (!arquivo)
    {
    ...
    PRODUTO tempProduto;
    while (fscanf(arquivo, "%d;%[^;];%f;%d;%d\n",
                  &tempProduto.Id, tempProduto.Nome,
                  &tempProduto.preco, &tempProduto.quantidade,
                  &tempProduto.id_categoria) == 5)
    {
        inserirnalista(&cabeca, tempProduto);
    }
    fclose(arquivo);
    return cabeca;
}

void removerproduto(noDuplo **cabeca, int id_produto_busca) // Remove um produto da lista duplamente encadeada
{
    if(*cabeca == NULL) // Verifica se a lista está vazia
    {
        perror("Não há produtos"); // Se não houver produtos, exibe mensagem de erro
        sleep(2);
        return;
    }
    noDuplo *atual = *cabeca; // Inicia o nó atual como a cabeça da lista

    while(atual->proximo != NULL && atual->produto.Id != id_produto_busca) // Enquanto houver próximo e o ID do produto atual não for o buscado
    {
        atual = atual->proximo; // Avança para o próximo nó
    }
    if (atual == NULL) // Se o nó atual for nulo, significa que não encontrou o produto
    {
        printf("ID %d Não encontrado em produtos\n", id_produto_busca); // Exibe mensagem de erro
        return;
    }
    if(atual == *cabeca) // Se o produto a ser removido for o primeiro da lista
    {
        *cabeca = atual->proximo; // Atualiza a cabeça da lista para o próximo nó
    }
    //ajusto que o anterior do atual aponte para o próximo do atual
    if(atual->anterior != NULL) // Verifica se o nó anterior não é nulo
    {
        atual->anterior->proximo = atual->proximo; // Atualiza o próximo do nó anterior para o próximo do nó atual
    }
    //ajusto que o proximo do atual aponte para o anterior do atual
    if(atual->proximo != NULL) // Verifica se o nó próximo não é nulo
    {
        atual->proximo->anterior= atual->anterior; // Atualiza o anterior do próximo nó para o anterior do nó atual
    }
...
int Adicionar_Carrinho(float *valor_total_compra, float *valor_total_limpeza, float *valor_total_alimentos, float *valor_total_Padaria, float *valor_total_Outros, float *valor_caixa, float *valor_inicial_caixa, noDuplo *cabeca)
{
    ...
    *valor_total_compra = 0;
    ...
    while (1)
    {
        ...
        noDuplo *produtoNo = buscarProdutoComCache(cabeca, escolha_id, NOME_ARQUIVO_PRODUTOS);
        if (!produtoNo)
        {
        ...
        *valor_total_compra += subtotal;
        ...
        printf("Deseja adicionar outro produto ao carrinho? (1 - Sim, 0 - Não): ");
        if (!verifica_int(&continuar) || (continuar != 0 && continuar != 1))
        {
            printf("Opção inválida!\n");
            sleep(2);
            return 0; // ou continue;
        }
...
void pagar(float *subtotal,float *valor_caixa)
{
    noDuplo *listaDupla = NULL;
...
int main()
{
    ...
    do
    {
        ...
        {
            listaDupla = carregarProdutosArquivo(NOME_ARQUIVO_PRODUTOS);
            if (listaDupla == NULL) {
            ...
            printf("1. Relatório em ordem alfabética\n");
            printf("2. Relatório por preço (menor para maior)\n");
            printf("3. Relatório por preço (maior para menor)\n");
            ...
            if (!verifica_int(&opcao))
            {
             ...
             switch (opcao) {
                    case 1:
                        ordenarNome(listaDupla);
                        listarProdutos(&listaDupla,NOME_ARQUIVO_PRODUTOS,ID_TODOS);
                        system("Pause");
                        break;
                    case 2:
                        ordenarPrecoCrescente(listaDupla);
                        listarProdutos(&listaDupla,NOME_ARQUIVO_PRODUTOS,ID_TODOS);
                        system("Pause");
                        break;
                    case 3:
                        ordenarPrecoDecrescente(listaDupla);
                        listarProdutos(&listaDupla,NOME_ARQUIVO_PRODUTOS,ID_TODOS);
                        system("Pause");
                        break;
                    case 4:
